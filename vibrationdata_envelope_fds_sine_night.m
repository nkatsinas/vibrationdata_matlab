...
%	   
        ...
        [f_sam,apsd_sam,~,slopec,sine_samp]=...
            env_generate_sample_psd_sine(n_ref,nbreak,npb,fn,slopec,initial,final,f1,f2,plateau,xsine);   
        kflag=1;
%
    else
%
        [f_sam,apsd_sam,max_sss,slopec,fs_samp,sine_samp]=...
        env_generate_sample_psd2_sine(nbreak,npb,xf,xapsd,slopec,f1,f2,plateau,xfs,xsine);
            kflag=2;
%
    end
...
out1=sprintf('  %s',output_name_psd);
disp(out1);

disp(' ');
disp(' Optimized Sine output name: ');
out1=sprintf('  %s',output_name_sine);
disp(out1);
