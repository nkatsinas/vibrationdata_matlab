%
%  plot_loglog_multiple_function_none_nlegend3.m  ver 1.2   By Tom Irvine
%
function[fig_num]=plot_loglog_multiple_function_none_nlegend3(fig_num,x_label,y_label,t_string,ppp,leg,fmin,fmax,nlegend)
%
f=ppp(:,1);
...
for i=1:ncols
    ...
    if(nlegend<=2)
        ...
        try
            ...
            try
                plot(f,ppp(:,(i+1)),'DisplayName',leg{i},'Color',cmap(i,:));        
            catch
            end
        end
        ...
        try
            if(i<=2)
                plot(f,ppp(:,(i+1)),'Color',cmap(i,:),'LineWidth',2.0);
            else
