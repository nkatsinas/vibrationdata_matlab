...
%
    ...
    [slope,grms] = calculate_PSD_slopes_no(freq,amp);    
...
%
    ...
    if( (total_rec>total_diff && max_sss<slopec && min_fff>(1/3)) || ik==1)      
%
  %      dB_rec=dB_diff;
...
%
        if(iu==1)
            ...
            fprintf('   Trial: total_diff=%8.4g  rms=%8.4g  \n',total_rec,std(sor_th));             
        end
...
if(iu==2)
    fprintf(' PSD: Freq(Hz) \t Accel(unit^2/Hz)  \n');    
end
...
if(iu==2)
    tstring=sprintf(' Sine-on-Random Level  %7.3g RMS overall',std(accel_th(:,2)));    
end
...
if(iu==1)
    ...
    [fig_num]=plot_double_y_SOR_other(fig_num,ppp1,ppp2,tstring,fmin,fmax,md);   
end
...
try
    listbox_octave=get(handles.listbox_octave,'Value');
    FDSEnvSOR.listbox_octave=listbox_octave;    
catch
end
...
try
    listbox_octave=FDSEnvSOR.listbox_octave;       
    set(handles.listbox_octave,'Value',listbox_octave); 
catch
end
