...
%  vrs_base.m  ver 1.0  by Tom Irvine

%    Vibration response spectrum for base excitation
%
%    SDOF model
%
...
%
%   accel_vrs has two columns:  
%       natural frequency (Hz) & acceleration response (GRMS)
%
%   supporting & plotting functions are included
%

function[accel_vrs]=vrs_base(Q,base_psd)
%
    tpi=2*pi;
    ...
    accel_vrs=zeros(nn,1);
    
    for j=1:nn
        
        den= (omn2(j)-om2) + (1i)*(2*damp*omegan(j)*omega);    
        num=omn2(j)+(1i)*2*damp*omega*omegan(j);
%
        ...
        [~,grms] = calculate_PSD_slopes_f(freq,response_psd);
    ...
    freq=fix_size(freq);
    accel_vrs=fix_size(accel_vrs);
    
    accel_vrs=[freq accel_vrs];
    
    generate_plot_vrs_response(accel_vrs,Q);
...
%   Generate Plot   
%
function[]=generate_plot_vrs_response(accel_vrs,Q)

    figure(1);
    plot(accel_vrs(:,1),accel_vrs(:,2));
        
    
    grid on;
    xlabel('Natural Frequency (Hz)');
    ylabel('Accel (GRMS)');
    out1=sprintf('Vibration Response Spectrum  Q=%g',Q);
    title(out1);
    
    set(gca,'MinorGridLineStyle',':','GridLineStyle','-','XScale','log',...
                     'YScale','log','XminorTick','off','YminorTick','off');
    
    ymax= 10^ceil(log10(max(accel_vrs(:,2))*1.2));
    ymin= 10^floor(log10(min(accel_vrs(:,2))*0.999));
    ...
    fstart=accel_vrs(1,1);
      fend=accel_vrs(end,1);
