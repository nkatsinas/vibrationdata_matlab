function varargout = vibrationdata_envelope_fds_spl_alt(varargin)
% VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT MATLAB code for vibrationdata_envelope_fds_spl_alt.fig
%      VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT, by itself, creates a new VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT returns the handle to a new VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT.M with the given input arguments.
%
%      VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT('Property','Value',...) creates a new VIBRATIONDATA_ENVELOPE_FDS_SPL_ALT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_envelope_fds_spl_alt_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_envelope_fds_spl_alt_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_envelope_fds_spl_alt

% Last Modified by GUIDE v2.5 21-Aug-2019 10:30:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_envelope_fds_spl_alt_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_envelope_fds_spl_alt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_envelope_fds_spl_alt is made visible.
function vibrationdata_envelope_fds_spl_alt_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_envelope_fds_spl_alt (see VARARGIN)

% Choose default command line output for vibrationdata_envelope_fds_spl_alt
...
% UIWAIT makes vibrationdata_envelope_fds_spl_alt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_envelope_fds_spl_alt_OutputFcn(hObject, eventdata, handles) 
...
delete(vibrationdata_envelope_fds_spl_alt);
...
%
   ...
   A=char(AA);
    ...
    for i=1:N
        Q(i)=str2double(A(k,:)); k=k+1;
    end

    for i=1:N
        bex(i)=str2double(A(k,:)); k=k+1;
    end
...
slopex=str2num(get(handles.edit_slope,'String'));
slopec=(slopex/10.)/log10(2.);
...
% Generate the sample psd
     ...
     if(ik<nnn || rand()<0.1)
        [f_sam,apsd_sam,spla,x,y]=...
        env_generate_sample_psd_spl_altq(n_ref,nbreak,npb,fn,ik,slopec,initial,final,f1,f2,fc,reference);            
     else
        [f_sam,apsd_sam,spla,x,y]=env_generate_sample_psd_spl_altq2(n_ref,nbreak,npb,fn,ik,slopec,initial,final,f1,f2,fc,reference,xr,yr);
        qflag=2;
     end
     
for iq=1:length(spla)
    if(isinf(spla(iq)))
        disp('** spla inf');
            pause(2)
    end    
...
%      Calculate the fds of the sample psd
    [fds_samfine,kflag]=env_fds_batch_spl_alt(apsd_samfine,n_ref,fn,damp,bex,T_out,iu,nmetric,kflag);
...
%      scale the psd
    ...
    apsd_samfine=apsd_samfine*scale;

    spla=spla+dBs;
    

     out1=sprintf('diff1=%8.4g dB  diff2=%8.4g dB  dBs=%8.4g',max(y)-min(y),max(spla)-min(spla),dBs);
%     disp(out1);    
    ...
    if(istrat<=3)
        [scale,error,error_max]=env_compare_alt_error_spl(n_ref,fds_ref,fds_samfine,bex);
    end
%       
%      calculate the prms value 
%             
    [prms]=env_grms_sam(nbreak,f_sam,apsd_sam);
%	 
% 
    ...
    slopet=zeros(nsx-1,1);
    
    for ij=1:nsx-1
        slopet(ij)=(spla(ij+1)-spla(ij));
    end
    max_slope=max(abs(slopet));
    ...
    if( (am<prmslow && max_slope<=slopex/3 ) || ik==1)      
...
%
        ...
        xspla=spla;
        setappdata(0,'xspla',xspla);
        
        nv=length(f_sam);
        power_spectral_density=[f_sam(1:nv) apsd_sam(1:nv)];
...
%
%        out1=sprintf('   Trial: drms=%10.4g  vrms=%10.4g  prms=%10.4g ',drms,vrms,prms); 

        oadb=20*log10(prms/reference);

        if(iu==1)
            out2=sprintf('  Record:  %8.4g psi rms    %8.4g dB   %d\n',prms_rec,oadb,qflag); 
        else
            out2=sprintf('  Record:  %8.4g Pa rms    %8.4g dB   %d\n',prms_rec,oadb,qflag);             
        end

        disp(out2); 

        out1=sprintf('istrat=%d   prmslow=%8.4g  \n',istrat,prmslow);
        disp(out1);
        
        spl_plot(hObject, eventdata, handles);
...
%         
[xapsdfine]=env_interp_best(nu,n_ref,xf,xapsd,fn);
%
...
[xfds,kflag]=env_fds_batch_spl_alt(xapsdfine,n_ref,fn,damp,bex,T_out,iu,nmetric,kflag);
%
% [prms]=env_prms_sam(nbreak,f_sam,xapsd);
%
...
if(iu==1)
    out1=sprintf(' Freq(Hz) \t Pressure(psi^2/Hz)  ');
else
    out1=sprintf(' Freq(Hz) \t Pressure(Pa^2/Hz)  ');    
end


disp(out1);

for i=1:length(power_spectral_density(:,1))
...
if(iu==1)
    out2=sprintf('  Record:  %8.4g psi rms    %8.4g dB   \n',prms_rec,oadb); 
else
    out2=sprintf('  Record:  %8.4g Pa rms    %8.4g dB   \n',prms_rec,oadb);             
end
...
if(iu==1)
    t_string=sprintf(' Power Spectral Density  Overall Level = %6.3g psi rms ',prms); 
else
    t_string=sprintf(' Power Spectral Density  Overall Level = %6.3g Pa rms ',prms);     
end
...
%
if(uiu==4)
    [fig_num,h3]=aco_fds_plot_2x2_alt_h2(fig_num,Q,bex,fn,ff,xx,xfds,fds_ref,nmetric,iu);
 ...
 %       
    ...
    [fig_num,h4]=aco_psd_fds_plot_2x2_alt_h2(fig_num,Q,bex,fn,xfds,...
...
output_name_SPL=sprintf('%s_spl',output_name);
output_name_SPL_3dB=sprintf('%s_spl_3dB',output_name);
...
SPL_3dB=getappdata(0,'SPL_3dB');


disp(' ');
disp(' Optimized SPL output name: '); 
out1=sprintf('  %s',output_name_SPL);
disp(out1);
out1=sprintf('  %s',output_name_SPL_3dB);
disp(out1);


assignin('base', output_name_SPL, SPL);
assignin('base', output_name_SPL_3dB, SPL_3dB);
...
[filename, pathname] = uigetfile('*_model.mat', 'Select plot save file');
...
function spl_plot(hObject, eventdata, handles)
...
fc=getappdata(0,'fc');
xspla=getappdata(0,'xspla');

fc=fix_size(fc);
xspla=fix_size(xspla);



SPL=[fc xspla];

SPL_3dB=[SPL(:,1) SPL(:,2)+3];
