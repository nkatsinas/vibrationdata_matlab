%
%   SSOR_synth_function.m  ver 1.0  by Tom Irvine
%

function[total_synth,fig_num]=SSOR_synth_function(psd,sine_sweep,fig_num,dur,rate)
...
unit='G';
aat=sprintf('Base Input Acceleration Time History Synthesis  %7.3g %s rms',std(amp),unit);
...
vvt=sprintf('Base Input Velocity Time History Synthesis  %7.3g %s rms',std(velox),unit);
...
ddt=sprintf('Base Input Displacement Time History Synthesis  %7.3g %s rms',std(dispx),unit);
