%
%   vibrationdata_wavelet_decomposition_engine_q.m  ver 1.1  October 26, 2015
%
%
...
alpha=zeros(nfr,1);
beta=zeros(nfr,1);
wavelet_up=zeros(nfr,1);
wavelet_low=zeros(nfr,1);
%
for ie=1:nfr
%
        progressbar(ie/nfr);
...
%
        ...
        [ax1r,ax2r,ax3r,ax4r,alphar,betar,wlr,wur]=...
...
%
        ...
        % Pre-calculate constants outside the loop for efficiency
        t1 = x4r(ie) + t(1);
        t2=t1 + tp*x3r(ie)/(2.*x2r(ie));
           
        % Logical indexing for efficient condition check
        mask = t >= t1 & t <= t2;
        
        % Vectorized calculations within the loop
        arg = x2r(ie) .* (t(mask) - t1);
        y = x1r(ie) .* sin(arg ./ double(x3r(ie))) .* sin(arg);
        
        % Update residual using logical indexing
        residual(mask) = residual(mask) - y;
...
for i=1:nfr
%
        fprintf(' amp=%10.4f   freq=%10.3f Hz   nhs=%d   delay=%10.4f \n',x1r(i),x2r(i)/tp,x3r(i),x4r(i));
%
        wavelet_table(i,1)=i;
