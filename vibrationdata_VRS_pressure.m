function varargout = vibrationdata_VRS_pressure(varargin)
% VIBRATIONDATA_VRS_PRESSURE MATLAB code for vibrationdata_VRS_pressure.fig
%      VIBRATIONDATA_VRS_PRESSURE, by itself, creates a new VIBRATIONDATA_VRS_PRESSURE or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_VRS_PRESSURE returns the handle to a new VIBRATIONDATA_VRS_PRESSURE or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_VRS_PRESSURE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_VRS_PRESSURE.M with the given input arguments.
%
%      VIBRATIONDATA_VRS_PRESSURE('Property','Value',...) creates a new VIBRATIONDATA_VRS_PRESSURE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_VRS_pressure_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_VRS_pressure_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_VRS_pressure

% Last Modified by GUIDE v2.5 14-Jul-2018 14:33:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_VRS_pressure_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_VRS_pressure_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_VRS_pressure is made visible.
function vibrationdata_VRS_pressure_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_VRS_pressure (see VARARGIN)

% Choose default command line output for vibrationdata_VRS_pressure
...
% UIWAIT makes vibrationdata_VRS_pressure wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_VRS_pressure_OutputFcn(hObject, eventdata, handles) 
...
delete(vibrationdata_VRS_pressure);
...
[~,applied_pressure_rms] = calculate_PSD_slopes(THM(:,1),THM(:,2));
...
a=a*area^2;  % convert pressure psd to force psd
[~,applied_force_rms]=calculate_PSD_slopes(f,a);
...
%
    ...
    [~,FT_rms_ind]=cm0(FT_psd,fi,n_ref);
...
for i=1:length(fn)
     ...
     trans_pressure_vrs_1_sigma(i,:)=[fn(i)  FT_rms(i)/area];
     ...
     trans_pressure_vrs_3_sigma(i,:)=[fn(i)  3*FT_rms(i)/area];          
end        
...
%
     ...
     trans_pressure_vrs_peak(j,:)=[fn(i)  ms*FT_rms(i)/area];
...
if(p==1)
    ...
    t_string=sprintf(' Input Force PSD  %8.4g lbf RMS',applied_force_rms);
   
else
    y_label='Force (N^2/Hz)'; 
    t_string=sprintf(' Input Force PSD  %8.4g N RMS',applied_force_rms);     
end
...
if(p==1)
    t_string=sprintf('Acceleration VRS, mass=%g lbm, Q=%g ',mass_orig,Q);
else
    t_string=sprintf('Acceleration VRS, mass=%g kg, Q=%g ',mass_orig,Q);       
end
...
if(p==1)
    t_string=sprintf('Velocity VRS, mass=%g lbm, Q=%g ',mass_orig,Q);
else
    t_string=sprintf('Velocity VRS, mass=%g kg, Q=%g ',mass_orig,Q);       
end
...
if(p==1)
    t_string=sprintf('Displacement VRS, mass=%g lbm, Q=%g ',mass_orig,Q);
else
    t_string=sprintf('Displacement VRS, mass=%g kg, Q=%g ',mass_orig,Q);       
end
...
if(p==1)
    t_string=sprintf('Transmitted Force VRS, mass=%g lbm, Q=%g ',mass_orig,Q);
else
    t_string=sprintf('Transmitted Force VRS, mass=%g kg, Q=%g ',mass_orig,Q);       
end
...
if(p==1)
    t_string=sprintf('Transmitted Pressure VRS, mass=%g lbm, Q=%g ',mass_orig,Q);
else
    t_string=sprintf('Transmitted Pressure VRS, mass=%g kg, Q=%g ',mass_orig,Q);       
end
