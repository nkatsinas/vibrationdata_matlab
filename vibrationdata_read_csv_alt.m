function varargout = vibrationdata_read_csv_alt(varargin)
% VIBRATIONDATA_READ_CSV_ALT MATLAB code for vibrationdata_read_csv_alt.fig
%      VIBRATIONDATA_READ_CSV_ALT, by itself, creates a new VIBRATIONDATA_READ_CSV_ALT or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_READ_CSV_ALT returns the handle to a new VIBRATIONDATA_READ_CSV_ALT or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_READ_CSV_ALT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_READ_CSV_ALT.M with the given input arguments.
%
%      VIBRATIONDATA_READ_CSV_ALT('Property','Value',...) creates a new VIBRATIONDATA_READ_CSV_ALT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_read_csv_alt_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_read_csv_alt_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_read_csv_alt

% Last Modified by GUIDE v2.5 17-Jun-2020 13:16:06

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_read_csv_alt_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_read_csv_alt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_read_csv_alt is made visible.
function vibrationdata_read_csv_alt_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_read_csv_alt (see VARARGIN)

% Choose default command line output for vibrationdata_read_csv_alt
...
% UIWAIT makes vibrationdata_read_csv_alt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_read_csv_alt_OutputFcn(hObject, eventdata, handles) 
...
delete(vibrationdata_read_csv_alt)
...
max_line=str2double(get(handles.edit_max_line,'String'));

N=max_line;

sarray = textscan(fid,'%s',N,'Delimiter','\n');
fclose(fid);
...
name=strtrim(get(handles.edit_array_name,'String'));

max_line=str2double(get(handles.edit_max_line,'String'));


irow=min([ irow max_line ]);
...
for j=1:icol
    ...
    out1=sprintf('j=%d icol=%d',j,icol);
    disp(out1);
    ...
    try
        output_name=sprintf('%s_%d',name,(j));
...
function edit_max_line_Callback(hObject, eventdata, handles)
% hObject    handle to edit_max_line (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_max_line as text
%        str2double(get(hObject,'String')) returns contents of edit_max_line as a double


% --- Executes during object creation, after setting all properties.
function edit_max_line_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_max_line (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
