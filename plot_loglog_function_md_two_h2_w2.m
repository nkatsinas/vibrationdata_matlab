%
%  plot_loglog_function_md_two_h2_w2.m  ver 1.3  by Tom Irvine
%
function[fig_num,h2]=plot_loglog_function_md_two_h2_w2(fig_num,x_label,...
...
if(min(a)>0 && min(b)>0 )
    ...
    plot(fa,a,fb,b,'LineWidth',2.0)
    legend(leg1,leg2);
    
else
    
    if(min(a)>0)
        plot(fa,a,'LineWidth',2.0)
        legend(leg1);        
    end
    if(min(b)>0)
        plot(fb,b,'LineWidth',2.0)
        legend(leg2);         
    end    
