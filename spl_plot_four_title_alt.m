%
%   spl_plot_four_title_alt.m  ver 1.9  by Tom Irvine
%
function[fig_num,hp]=spl_plot_four_title_alt(fig_num,n_type,f1,dB1,f2,dB2,f3,dB3,f4,dB4,leg1,leg2,leg3,leg4,tstring,stype)
%
...
if(stype<=2)
    ...
    leg4=sprintf('%s  oafpl %6.4g dB',leg4,oaspl_4);
...
plot(f4,dB4,'Color',cmap(5,:),'DisplayName',leg4);
...
fmin=max([ min(f1) min(f2) min(f3) min(f4)]);
fmax=min([ max(f1) max(f2) max(f3) max(f4)]);
