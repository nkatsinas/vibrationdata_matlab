...
%  beam_FEA_base_arbitrary_BC1_HS.m  ver 1.1  by Tom Irvine

function[accel,rel_disp,stress,fig_num]=...
               beam_FEA_base_arbitrary_BC1_HS(ndof,acc,ddd,dt,fn,iu,nde,dx,nt,tpi,cna,E,fig_num,THM)
...
if(iu==1)
    t_string=sprintf('Relative Displacement at Beam Free End \n max=%7.3g in  min=%7.3g in  %7.3g in RMS',srdmax,srdmin,srd);
    ylabel('Rel Disp (in)');
else
...
disp(' ');
if(iu==1)
    disp('  Beam Free End Pseudo Velocity (in/sec) from Fundamental Mode');
    sss=386;
else
    disp('  Beam Free End Pseudo Velocity (m/sec) from Fundamental Mode');
    sss=1;
end

aaa=max(abs(acc(:,ndof-1)));
PV=sss*aaa/(2*pi*fn(1));
...
disp(' ');
if(iu==1)
    disp('  Beam Free End Relative Velocity (in/sec)');
else
    disp('  Beam Free End Relative Velocity (m/sec)');
end
disp('  ');
disp('      Max    Min    RMS ');
...
disp(' ');
if(iu==1)
    disp('  Beam Free End Relative Displacement (in) ');
else
    disp('  Beam Free End Relative Displacement (mm) ');
end
disp('  ');
disp('      Max    Min    RMS ');
