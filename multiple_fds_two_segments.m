function varargout = multiple_fds_two_segments(varargin)
% MULTIPLE_FDS_TWO_SEGMENTS MATLAB code for multiple_fds_two_segments.fig
%      MULTIPLE_FDS_TWO_SEGMENTS, by itself, creates a new MULTIPLE_FDS_TWO_SEGMENTS or raises the existing
%      singleton*.
%
%      H = MULTIPLE_FDS_TWO_SEGMENTS returns the handle to a new MULTIPLE_FDS_TWO_SEGMENTS or the handle to
%      the existing singleton*.
%
%      MULTIPLE_FDS_TWO_SEGMENTS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MULTIPLE_FDS_TWO_SEGMENTS.M with the given input arguments.
%
%      MULTIPLE_FDS_TWO_SEGMENTS('Property','Value',...) creates a new MULTIPLE_FDS_TWO_SEGMENTS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before multiple_fds_two_segments_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to multiple_fds_two_segments_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help multiple_fds_two_segments

% Last Modified by GUIDE v2.5 16-May-2019 12:37:40

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @multiple_fds_two_segments_OpeningFcn, ...
                   'gui_OutputFcn',  @multiple_fds_two_segments_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before multiple_fds_two_segments is made visible.
function multiple_fds_two_segments_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to multiple_fds_two_segments (see VARARGIN)

% Choose default command line output for multiple_fds_two_segments
...
try
    ...
    out=sprintf('nflight=%d line 96',nflights);
    disp(out);
    set(handles.listbox_num,'Value',nflights);
catch
    disp('no nflights 100');
end
...
try
    out=sprintf('nflight=%d line 114',nflights);
    disp(out); 
    set(handles.listbox_num,'Value',nflights);
catch
    disp('no nflights 116');
end
...
% UIWAIT makes multiple_fds_two_segments wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = multiple_fds_two_segments_OutputFcn(hObject, eventdata, handles) 
...
delete(multiple_fds_two_segments);
    ...
    for j=1:m
        ...
        if(~isempty(p9550) && ~isempty(p9550_lognormal)) 
            try
                ...
                name3=sprintf('%s_p9550_lognormal',NNN);
                name3=strrep(name3,' ','_');        
                assignin('base',name3,[fn p9550_lognormal]);   
        
                Np9550_lognormal{ivk3}=name3;                   
                ivk3=ivk3+1;               
