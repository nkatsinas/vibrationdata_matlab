function varargout = vibrationdata_wavelet_synth_alt(varargin)
% VIBRATIONDATA_WAVELET_SYNTH_ALT MATLAB code for vibrationdata_wavelet_synth_alt.fig
%      VIBRATIONDATA_WAVELET_SYNTH_ALT, by itself, creates a new VIBRATIONDATA_WAVELET_SYNTH_ALT or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_WAVELET_SYNTH_ALT returns the handle to a new VIBRATIONDATA_WAVELET_SYNTH_ALT or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_WAVELET_SYNTH_ALT('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_WAVELET_SYNTH_ALT.M with the given input arguments.
%
%      VIBRATIONDATA_WAVELET_SYNTH_ALT('Property','Value',...) creates a new VIBRATIONDATA_WAVELET_SYNTH_ALT or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_wavelet_synth_alt_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_wavelet_synth_alt_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_wavelet_synth_alt

% Last Modified by GUIDE v2.5 05-Jul-2019 15:29:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_wavelet_synth_alt_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_wavelet_synth_alt_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_wavelet_synth_alt is made visible.
function vibrationdata_wavelet_synth_alt_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_wavelet_synth_alt (see VARARGIN)

% Choose default command line output for vibrationdata_wavelet_synth_alt
...
% UIWAIT makes vibrationdata_wavelet_synth_alt wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_wavelet_synth_alt_OutputFcn(hObject, eventdata, handles) 
...
%
    ...
    out2=sprintf('   Trial     Accel       Vel          Disp      T.Error      I.Error   kt   dskw');
%
    disp(out1);
    disp(out2);
%                
...
%%%        if(inn<nx || rand()<0.6)
        ...
        if(1)
            if(istrat==1) % reverse sine sweep
                [amp,td,nhs,stype]=ws_synth3b(amp_start,dur,onep5_period,ntrials,inn,f,nspec);
            end    
            if(istrat==2) % random
            ...
            [amp,td,nhs,stype]=ws_synth999(amp_start,dur,onep5_period,inn,f,nspec,store_amp,store_NHS,store_td,iwin);              
        end
...
%				
            if(nv>= 10 && abs(irror) >= 4.0*irec)
                break;
            end
            if(nv>= 20 && abs(irror) >= 1.5*irec)
                break;
            end
            if(nv>= 50 && irror >= errorbefore)
                break;
            end
%
...
%
        ...
        points=ymax*vmax*dmax*re1*re2*kt*dskk;
        

                    
        if(inn==1 || points < points_before)
            out1=sprintf('      %ld %10.2f %12.3f %12.3f %11.2f %10.2f  %4.2f  %4.2f',inn,ymax,vmax,dmax,re1,re2,kt,abs(dsk));
            disp(out1);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
delete(vibrationdata_wavelet_synth_alt);
