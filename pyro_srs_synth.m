function varargout = pyro_srs_synth(varargin)
% PYRO_SRS_SYNTH MATLAB code for pyro_srs_synth.fig
%      PYRO_SRS_SYNTH, by itself, creates a new PYRO_SRS_SYNTH or raises the existing
%      singleton*.
%
%      H = PYRO_SRS_SYNTH returns the handle to a new PYRO_SRS_SYNTH or the handle to
%      the existing singleton*.
%
%      PYRO_SRS_SYNTH('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PYRO_SRS_SYNTH.M with the given input arguments.
%
%      PYRO_SRS_SYNTH('Property','Value',...) creates a new PYRO_SRS_SYNTH or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before pyro_srs_synth_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to pyro_srs_synth_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help pyro_srs_synth

% Last Modified by GUIDE v2.5 24-Oct-2015 19:37:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @pyro_srs_synth_OpeningFcn, ...
                   'gui_OutputFcn',  @pyro_srs_synth_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before pyro_srs_synth is made visible.
function pyro_srs_synth_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to pyro_srs_synth (see VARARGIN)

% Choose default command line output for pyro_srs_synth
...
% UIWAIT makes pyro_srs_synth wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = pyro_srs_synth_OutputFcn(hObject, eventdata, handles) 
...
setappdata(0,'accleration_1',acceleration);
setappdata(0,'velocity_1',velocity);
setappdata(0,'displacement_1',displacement);
setappdata(0,'wavelet_table_1',wavelet_table);
setappdata(0,'wavelet_low_1',wavelet_low);
setappdata(0,'wavelet_up_1',wavelet_up);
setappdata(0,'alpha_1',alpha);
setappdata(0,'beta_1',beta);

else

    acceleration=getappdata(0,'accleration_1');
    velocity=getappdata(0,'velocity_1');
    displacement=getappdata(0,'displacement_1');
    wavelet_table=getappdata(0,'wavelet_table_1');
    wavelet_low=getappdata(0,'wavelet_low_1');
    wavelet_up=getappdata(0,'wavelet_up_1');
    alpha=getappdata(0,'alpha_1');
    beta=getappdata(0,'beta_1');
    
end

ichoice=1;

setappdata(0,'ichoice',ichoice);
...
delay_max=str2num(get(handles.edit_delay_max,'String'));
...
if(n==1)
    ...
    set(handles.edit_dlimit,'String','1');    
else
    set(handles.text_dlimit,'String','mm');  
    set(handles.edit_dlimit,'String','25.4');       
end
...
% --- Executes on button press in pushbutton_decomp.
function pushbutton_decomp_Callback(hObject, eventdata, handles)
% hObject    handle to pushbutton_decomp (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
function edit_delay_max_Callback(hObject, eventdata, handles)
% hObject    handle to edit_delay_max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_delay_max as text
%        str2double(get(hObject,'String')) returns contents of edit_delay_max as a double


% --- Executes during object creation, after setting all properties.
function edit_delay_max_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_delay_max (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
