function varargout = multiple_fds_four_events(varargin)
% MULTIPLE_FDS_FOUR_EVENTS MATLAB code for multiple_fds_four_events.fig
%      MULTIPLE_FDS_FOUR_EVENTS, by itself, creates a new MULTIPLE_FDS_FOUR_EVENTS or raises the existing
%      singleton*.
%
%      H = MULTIPLE_FDS_FOUR_EVENTS returns the handle to a new MULTIPLE_FDS_FOUR_EVENTS or the handle to
%      the existing singleton*.
%
%      MULTIPLE_FDS_FOUR_EVENTS('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MULTIPLE_FDS_FOUR_EVENTS.M with the given input arguments.
%
%      MULTIPLE_FDS_FOUR_EVENTS('Property','Value',...) creates a new MULTIPLE_FDS_FOUR_EVENTS or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before multiple_fds_four_events_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to multiple_fds_four_events_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help multiple_fds_four_events

% Last Modified by GUIDE v2.5 09-Aug-2019 11:03:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @multiple_fds_four_events_OpeningFcn, ...
                   'gui_OutputFcn',  @multiple_fds_four_events_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before multiple_fds_four_events is made visible.
function multiple_fds_four_events_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to multiple_fds_four_events (see VARARGIN)

% Choose default command line output for multiple_fds_four_events
...
% UIWAIT makes multiple_fds_four_events wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = multiple_fds_four_events_OutputFcn(hObject, eventdata, handles) 
...
delete(multiple_fds_four_events);
...
%        fds_th4=zeros(nf,nc);
        ...
        [~, i5] = min(abs(t-start_time(i,3)));    
        [~, i6] = min(abs(t-end_time(i,3)));
        [~, i7] = min(abs(t-start_time(i,4)));    
        [~, i8] = min(abs(t-end_time(i,4)));           
        ...
        disp(' segment 3');
        yy3=amp(i5:i6);
        [fds_th3]=fds_engine(fds_th,yy3,nf,nQ,Q,nbex,bex,fn,dt);
        clear yy3;
        
        disp(' segment 4');
        yy4=amp(i7:i8);
        [fds_th4]=fds_engine(fds_th,yy4,nf,nQ,Q,nbex,bex,fn,dt);
        clear yy4;        
...
%%%
    ...
    if(min(qqq)==0)
        ...
        for ij=1:nQ
            for ik=1:nbex
                ...
                nnn=sprintf('%s_ev1',name{i,kv});
                data=[fn fds_th1(:,kv)];
                assignin('base',nnn,data);
                
                nnn=sprintf('%s_ev2',name{i,kv});                
                data=[fn fds_th2(:,kv)];
                assignin('base',nnn,data);                
                
                nnn=sprintf('%s_ev3',name{i,kv});                
                data=[fn fds_th3(:,kv)];
                assignin('base',nnn,data);
                
                nnn=sprintf('%s_ev4',name{i,kv});                
                data=[fn fds_th4(:,kv)];
                assignin('base',nnn,data);                 
    ...
    for kv=1:nc
        out1=sprintf('  %s_ev1 ',name{i,kv});
        disp(out1);
        out1=sprintf('  %s_ev2 ',name{i,kv});
        disp(out1);
        out1=sprintf('  %s_ev3 ',name{i,kv});
        disp(out1);
        out1=sprintf('  %s_ev4 ',name{i,kv});
        disp(out1);        
    end    
...
%        end    
    ...
    for i=1:N
        a=A(k,:); k=k+1;
        start_time(i,4) = str2double(strtrim(a));
    end    
    
    for i=1:N
        a=A(k,:); k=k+1;
        end_time(i,4) = str2double(strtrim(a));
    end       
