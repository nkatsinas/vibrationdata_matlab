function varargout = psd_max_interp(varargin)
% PSD_MAX_INTERP MATLAB code for psd_max_interp.fig
%      PSD_MAX_INTERP, by itself, creates a new PSD_MAX_INTERP or raises the existing
%      singleton*.
%
%      H = PSD_MAX_INTERP returns the handle to a new PSD_MAX_INTERP or the handle to
%      the existing singleton*.
%
%      PSD_MAX_INTERP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PSD_MAX_INTERP.M with the given input arguments.
%
%      PSD_MAX_INTERP('Property','Value',...) creates a new PSD_MAX_INTERP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before psd_max_interp_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to psd_max_interp_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help psd_max_interp

% Last Modified by GUIDE v2.5 16-Dec-2019 13:28:23

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @psd_max_interp_OpeningFcn, ...
                   'gui_OutputFcn',  @psd_max_interp_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before psd_max_interp is made visible.
function psd_max_interp_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to psd_max_interp (see VARARGIN)

% Choose default command line output for psd_max_interp
...
% UIWAIT makes psd_max_interp wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = psd_max_interp_OutputFcn(hObject, eventdata, handles) 
...
for i=1:num
    ...
    [~,grms(i)] = calculate_PSD_slopes_no(f,a);
    ...
    try
        ...
        warndlg('Error:  interpolate_PSD_arbitary_frequency');
        return;
    end
...
plot(THM(:,1),THM(:,2),'color',cmap(1,:),'DisplayName',leg{1});
...
for i=1:num
    ...
    plot(f,a,'color',cmap(i+1,:),'DisplayName',leg{i+1});
end    
