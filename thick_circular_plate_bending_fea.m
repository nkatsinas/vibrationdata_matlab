function varargout = thick_circular_plate_bending_fea(varargin)
% THICK_CIRCULAR_PLATE_BENDING_FEA MATLAB code for thick_circular_plate_bending_fea.fig
%      THICK_CIRCULAR_PLATE_BENDING_FEA, by itself, creates a new THICK_CIRCULAR_PLATE_BENDING_FEA or raises the existing
%      singleton*.
%
%      H = THICK_CIRCULAR_PLATE_BENDING_FEA returns the handle to a new THICK_CIRCULAR_PLATE_BENDING_FEA or the handle to
%      the existing singleton*.
%
%      THICK_CIRCULAR_PLATE_BENDING_FEA('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in THICK_CIRCULAR_PLATE_BENDING_FEA.M with the given input arguments.
%
%      THICK_CIRCULAR_PLATE_BENDING_FEA('Property','Value',...) creates a new THICK_CIRCULAR_PLATE_BENDING_FEA or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before thick_circular_plate_bending_fea_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to thick_circular_plate_bending_fea_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help thick_circular_plate_bending_fea

% Last Modified by GUIDE v2.5 27-Jul-2022 14:45:05

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @thick_circular_plate_bending_fea_OpeningFcn, ...
                   'gui_OutputFcn',  @thick_circular_plate_bending_fea_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before thick_circular_plate_bending_fea is made visible.
function thick_circular_plate_bending_fea_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to thick_circular_plate_bending_fea (see VARARGIN)

% Choose default command line output for thick_circular_plate_bending_fea
...
function varargout = thick_circular_plate_bending_fea_OutputFcn(hObject, eventdata, handles) 
...
if(handles.unit==1)
   ...
   set(handles.diametertext,'String','Width(in)'); 
   ...
   set(handles.diametertext,'String','Width(mm)');
...
diam=str2num(get(handles.diametereditbox,'String'));
...
fprintf(' E=%8.4g  mu=%7.3g  total mass=%8.4g  \n',E,mu,mass);
fprintf(' Diam=%8.4g  thick=%8.4g  \n\n',diam,thick);
...
if(leftBC==2)
    structuralBC(model,'Edge',1,'XDisplacement',0,'YDisplacement',0,'ZDisplacement',0);         
end
...
if(N>=7)
    sss=sprintf(' %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g ',fn(1),fn(2),fn(3),fn(4),fn(5),fn(6),fn(7));
end
if(N>=8)
    sss=sprintf(' %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g \n %8.4g ',fn(1),fn(2),fn(3),fn(4),fn(5),fn(6),fn(7),fn(8));
end
...
function diametereditbox_Callback(hObject, eventdata, handles)
% hObject    handle to diametereditbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of diametereditbox as text
%        str2double(get(hObject,'String')) returns contents of diametereditbox as a double
clear_answer(hObject, eventdata, handles);
...
% --- Executes during object creation, after setting all properties.
function diametereditbox_CreateFcn(hObject, eventdata, handles)
% hObject    handle to diametereditbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on key press with focus on diametereditbox and none of its controls.
function diametereditbox_KeyPressFcn(hObject, eventdata, handles)
% hObject    handle to diametereditbox (see GCBO)
% eventdata  structure with the following fields (see UICONTROL)
%	Key: name of the key that was pressed, in lower case
%	Character: character interpretation of the key(s) that was pressed
