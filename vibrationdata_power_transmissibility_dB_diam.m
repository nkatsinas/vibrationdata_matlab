function varargout = vibrationdata_power_transmissibility_dB_diam(varargin)
% VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM MATLAB code for vibrationdata_power_transmissibility_dB_diam.fig
%      VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM, by itself, creates a new VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM or raises the existing
%      singleton*.
%
%      H = VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM returns the handle to a new VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM or the handle to
%      the existing singleton*.
%
%      VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM.M with the given input arguments.
%
%      VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM('Property','Value',...) creates a new VIBRATIONDATA_POWER_TRANSMISSIBILITY_DB_DIAM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before vibrationdata_power_transmissibility_dB_diam_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to vibrationdata_power_transmissibility_dB_diam_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help vibrationdata_power_transmissibility_dB_diam

% Last Modified by GUIDE v2.5 27-Nov-2017 16:38:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @vibrationdata_power_transmissibility_dB_diam_OpeningFcn, ...
                   'gui_OutputFcn',  @vibrationdata_power_transmissibility_dB_diam_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before vibrationdata_power_transmissibility_dB_diam is made visible.
function vibrationdata_power_transmissibility_dB_diam_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to vibrationdata_power_transmissibility_dB_diam (see VARARGIN)

% Choose default command line output for vibrationdata_power_transmissibility_dB_diam
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes vibrationdata_power_transmissibility_dB_diam wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = vibrationdata_power_transmissibility_dB_diam_OutputFcn(hObject, eventdata, handles) 
...
clear length;
%
FS1=strtrim(get(handles.edit_array_1,'String'));
THM1=evalin('base',FS1);
%
FS2=strtrim(get(handles.edit_array_2,'String'));
...
[fig_num]=trans_dB_plot_fd(fig_num,ff,ab,stitle);
