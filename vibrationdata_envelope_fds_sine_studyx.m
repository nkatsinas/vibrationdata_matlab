        ...
        if(rand()>0.5)
            ...
            if(npb~=5)
                ...
                if(rand()>0.5)
                    ...
                    fs_samp(2)=xf(2)*(0.98+0.04*rand());
                    fs_samp(3)=xf(3)*(0.98+0.04*rand());
                    fs_samp=sort(fs_samp);
                    kflag=5;
                end    
            end
        end
...
%      Interpolate the sample psd
    nnbreak=length(f_sam);
    [fn,apsd_samfine]=env_interp_sam(f_sam,apsd_sam,nnbreak,n_ref,fn);
% 
%
    ...
    interp_psdin=apsd_samfine;

%      Calculate the fds of the sample psd
     nnn=length(interp_psdin);
    
    [fds_samfine]=env_fds_batch_alt(interp_psdin,nnn,fn,damp,bex,T_out,iu,nmetric);
%
    [fds_sine_samp]=sine_fds(fn,damp,bex,fs_samp,sine_samp,T_out);
    
    fds_samfine=fds_sine_samp+fds_samfine;   % rough approximation
...
%             
    ...
    for ij=1:length(sine_samp)
        gs2=gs2+(sine_samp(ij)/sqrt(2));
        vs2=vs2+386*(sine_samp(ij))/sqrt(2)/(tpi*fs_samp(ij));
        ds2=ds2+386*(sine_samp(ij))/sqrt(2)/(tpi*fs_samp(ij))^2;        
    end
    
    grms=sqrt(grms^2+gs2);
    vrms=sqrt(vrms^2+vs2);
    drms=sqrt(drms^2+ds2);    
%	 
...
%
    ...
    if( (max_sss<slopec && min_fff>(1/3) && z<total_record) || ik==1)
...
%
        fprintf('   Trial: %8.4g  drms=%8.4g  vrms=%8.4g  grms=%8.4g  \n',total_record,drms,vrms,grms);  
