...
%
     omega_j=atan2(y,x);
     omega_k=atan2(z,(sqrt(x^2+y^2)));
     omega_l=0;
%
     omega_k=-omega_k;
%
     cos_omega_j=cos(omega_j);
     sin_omega_j=sin(omega_j);
%
     cos_omega_k=cos(omega_k);
     sin_omega_k=sin(omega_k);
%
     cos_omega_l=cos(omega_l);
     sin_omega_l=sin(omega_l);
%
     omega_oj(1,:)=[cos_omega_j  -sin_omega_j   0]; 
     omega_oj(2,:)=[sin_omega_j   cos_omega_j   0]; 
     omega_oj(3,:)=[     0            0         1]; 
%
     omega_jk(1,:)=[cos_omega_k   0  sin_omega_k ];
     omega_jk(2,:)=[        0     1       0      ];
     omega_jk(3,:)=[-sin_omega_k  0  cos_omega_k];
%
     omega_kl(1,:)=[1        0           0       ];
     omega_kl(2,:)=[0   cos_omega_l  -sin_omega_l];
     omega_kl(3,:)=[0   sin_omega_l   cos_omega_l];
%
     tm_ol=omega_oj*omega_jk*omega_kl;
     tm_ol=tm_ol';
%
     clear T;
     T=zeros(12,12);
