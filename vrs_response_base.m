...
%  vrs_response_base.m  ver 1.0  by Tom Irvine
...
%     damp = viscous damping ratio  (typical value is 0.05)
%
...
function[response_psd]=sdof_psd_response_base(fn,damp,base_psd)
%
    tpi=2*pi;
...
%
    ...
    generate_plot_psd_response(base_psd,response_psd,fn,damp);
...
%   Generate Plot   
%
function[]=generate_plot_psd_response(base_psd,response_psd,fn,damp)
    ...
    xlabel('Frequency (Hz)');
    ylabel('Accel (G^2/Hz)');
    out1=sprintf('PSD  SDOF Response, fn=%g Hz, damp=%g',fn,damp);
    title(out1);
